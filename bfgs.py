...
def curr_error_grad(w_matrix, w_ho, batchset, step):
	...
	#test condition 1 of Wolfe conditioins
...
def updatew(alpha, direction): #direction is a column vector
	...
	newmatrix = newmatrix + alpha * direction[:-superpara.NUM_HIDDEN, 0].reshape(newmatrix.shape) #cut the last num_hidden in direction
...
def linsearch(batchset, step, direction, curr_error, curr_gradient):
	...
	# itr = 0
	# while not(flag1 and flag2) and itr < 3:
