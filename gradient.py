...
def hidden_output(x):
	return activation.activation_fun(hidden_input(x))[:, 0] #vector activation function applied to hidden input
								   #[:, 0] is used to extract the array from the (n, 1) shape vector
...
#candidate solution
def sol_candidate(x):
	return pipes.init(x) + (x[1, 0] - superpara.T_START) * nn_output(x) #the initial value is x[0, 0] when t equals T_START
